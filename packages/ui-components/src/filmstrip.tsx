...
import { Badge, FieldLabel } from "./primitives.js";
  ...
  return (
    <div className={cn("rounded-[24px] border p-3", shellClass)}>
      <div className="mb-3 flex items-center justify-between">
        <FieldLabel>{tone === "dark" ? "Gallery Strip" : "Filmstrip"}</FieldLabel>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        {photos.map((record) => (
          <button
            className={cn(
              "group relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl border transition",
