...
Expected artifacts for version `0.1.12`:

- `dist/flutter-release/linux/chronopic-flutter-linux-x64-0.1.12.tar.gz`
- `dist/flutter-release/linux/chronopic-flutter-linux-x64-0.1.12.tar.gz.sha256`
...
`v0.1.7` note:
the tag-triggered Release workflow completed successfully for all three jobs:
`prepare-release`,
`flutter-android`,
and `flutter-linux`.
...
`v0.1.9` note:
the tag-triggered Release workflow completed successfully as run `25672015701`
for all three jobs:
`prepare-release`,
`flutter-android`,
and `flutter-linux`.
...
and all matching checksum assets.
Post-publication verification downloaded the GitHub Release assets into
`.tmp/release-verify/v0.1.9`,
checked all three sha256 files,
and confirmed the Linux tarball contains the `chronopic` executable.

`v0.1.10` note:
the tag-triggered Release workflow completed successfully as run `25685916285`
for all three jobs:
`prepare-release`,
`flutter-android`,
and `flutter-linux`.
...
`v0.1.12` note:
the tag-triggered Release workflow completed successfully as run `25949420131`
for all three jobs:
`prepare-release`,
`flutter-android`,
and `flutter-linux`.
